...
def pdb2sas(pdb, q=np.linspace(0,0.5,501),return_pr=True):
    ...
    for i in range(natoms):
        ff[i,:] = formfactor(pdb.atomtype[i],q=qsh)
    if numba:
...
if __name__ == "__main__":
    ...
    q = np.linspace(0,4,1001)
    Iq, Pr = pdb2sas(pdb,q=q,return_pr=True)
    # Pr2 = realspace_debye(pdb,r=Pr[:,0])
    end = time.time()
    print("Total calculation time (sec): %s" % (end-start))
    np.savetxt(basename+'.Iq.dat',Iq,delimiter=' ',fmt='%.8e')
    np.savetxt(basename+'.Pr.dat',Pr,delimiter=' ',fmt='%.8e')
    # plt.plot(Pr[:,0],Pr[:,1])
    # plt.plot(Pr2[:,0],Pr2[:,1])
    # plt.show()
