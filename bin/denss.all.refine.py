...
def multi_denss(niter, **kwargs):
    try:
        ...
        # This might be messy...but it might work.
        args.rho_known, rhok_side = saxs.read_mrc('../../A_alignedpdb2mrc/A_aligned_s210_n64_mslow_solv0.mrc')
        rhok_nsamples = args.rho_known.shape[0]
        rhok_voxel = rhok_side/rhok_nsamples
        print("Did rho_known get read?")
        print("\n%s"%rhok_side)
        print("That was rhok_side of rho_kn (known)")
        # if (not np.isclose(rhok_side, args.side) or
        #    not np.isclose(rhok_voxel, args.voxel) or
        #    not np.isclose(rhok_nsamples, args.nsamples)):
        #    print("rho_known (known) density dimensions do not match given options.")
        #    print("Oversampling and voxel size adjusted to match rhok_start dimensions.")
        
        result = saxs.denss(**kwargs, rho_known = args.rho_known)
        logger.info('END')
        return result
        time.sleep(1)
    except KeyboardInterrupt:
        logger.info('There was a keyboard interupt to multidense, does this keep going?: %s, %s', nmaps, niter)
        time.sleep(1)
        # I'm hoping the sleep gives it a moment to put the output.
        # for reasons I can't find in the documentation, sys exit (5)
        # kills 5 instances of dense (so you iteration 0, 5, 10, etc).
        # but putting two exit statements in a row kills the whole thing 
        # although the 'loop' seems to continue to run and print the next
        # iterations output file before the second exit catches up with it,
        # so I'm trying a sleep there, as well.
        sys.exit(1)
        time.sleep(1)
        sys.exit(1)
        # pass
